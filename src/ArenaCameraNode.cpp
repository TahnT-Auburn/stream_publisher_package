...
void ArenaCameraNode::parse_parameters_()
{
  ...
  try {
    ...
    nextParameterToDeclare = "stream_auto_negotiate_packet_size";
    ...
    nextParameterToDeclare = "stream_packet_resend_enable";
    ...
    this->get_parameter("trigger_mode", trigger_mode_activated_);
    ...
    this->declare_parameter("qos_history", "");
    this->get_parameter("qos_history", pub_qos_history_);
    is_passed_pub_qos_history_ = pub_qos_history_ != "";

    nextParameterToDeclare = "qos_history_depth";
    this->declare_parameter("qos_history_depth", 0);
    this->get_parameter("qos_history_depth", pub_qos_history_depth_);
    is_passed_pub_qos_history_depth_ = pub_qos_history_depth_ > 0;

    nextParameterToDeclare = "qos_reliability";
    this->declare_parameter("qos_reliability", "");
    this->get_parameter("qos_reliability", pub_qos_reliability_);
  ...
  //RCLCPP_INFO(this->get_logger(), "offset_x: %d", offset_x_);
  //RCLCPP_INFO(this->get_logger(), "offset_y: %d", offset_y_);
  ...
  RCLCPP_INFO(this->get_logger(), "trigger_mode: %d", trigger_mode_activated_);
  RCLCPP_INFO(this->get_logger(), "topic: %s", topic_.c_str());
  RCLCPP_INFO(this->get_logger(), "qos_history: %s", pub_qos_history_.c_str());
  RCLCPP_INFO(this->get_logger(), "qos_history_depth: %d", pub_qos_history_depth_);
  RCLCPP_INFO(this->get_logger(), "qos_reliability: %s\n", pub_qos_reliability_.c_str());
...
void ArenaCameraNode::set_nodes_()
{
  ...
  set_nodes_exposure_();
  set_nodes_auto_negotiate_packet_size_();
  set_nodes_packet_resend_enable_();
...
void ArenaCameraNode::set_nodes_roi_()
{
  ...
  //TODO: Max ROI is set mannually, have it set through Arena for other camera/lense combinations
  int64_t width_max_ = 1936;
  int64_t height_max_ = 1464;
  
  size_t offset_x_ = (width_max_ - width_)/2;
  size_t offset_y_ = (height_max_ - height_)/2;
...
void ArenaCameraNode::set_nodes_auto_negotiate_packet_size_()
{
  if (stream_auto_negotiate_packet_size_)
  {
    auto nodemap = m_pDevice->GetTLStreamNodeMap();
    Arena::SetNodeValue<bool>(nodemap, "StreamAutoNegotiatePacketSize", stream_auto_negotiate_packet_size_);
  }
}

void ArenaCameraNode::set_nodes_packet_resend_enable_()
{
  if (stream_packet_resend_enable_)
  {
    auto nodemap = m_pDevice->GetTLStreamNodeMap();
    Arena::SetNodeValue<bool>(nodemap, "StreamPacketResendEnable", stream_packet_resend_enable_);
  }
}
